...
async def analyze_pic(update: Update | None, context: ContextTypes.DEFAULT_TYPE, direct_chat_id: int | None = None, direct_user: User | None = None, direct_file_id: str | None = None) -> None:
    ...
    try: # Скачивание и вызов ИИ
        ...
        image_prompt_text = (
            f"Ты - остроумный и ОЧЕНЬ саркастичный комментатор с хорошим чувством юмора (иногда черным). Посмотри ВНИМАТЕЛЬНО на приложенную КАРТИНКУ. Твоя задача - написать **КОРОТКИЙ (1-3 предложения)**, смешной и **ЯЗВИТЕЛЬНЫЙ** комментарий о том, что на ней изображено (о сюжете, действии, объектах). Используй иронию, сарказм, абсурдные сравнения. Можешь использовать **НЕМНОГО УМЕСТНОГО МАТА** для усиления эффекта, но без прямой ненависти или тупой агрессии.\n\n"
            f"НЕ пиши вступлений. Начинай ответ с `🗿 `.\n\n"
            f"Пример (кот в коробке): '🗿 Ого, Его Коробчество собственной персоной! Видимо, размышляет о бренности бытия... или просто где бы еще насрать.'\n"
            f"Пример (пикник): '🗿 Классический выезд на природу: пластиковые стаканчики, выражение вселенского счастья на лицах и, наверняка, забытый мусор где-то за кадром.'\n"
            f"Пример (смешная собака): '🗿 Какая интересная порода! Похоже на результат смелого генетического эксперимента или просто очень креативной пьянки.'\n\n"
            f"Твой ЯЗВИТЕЛЬНЫЙ комментарий к приложенной картинке:"
        )
        base64_image = base64.b64encode(photo_bytes).decode('utf-8')
        messages_for_api = [{
            "role": "user",
            "content": [
                {"type": "text", "text": image_prompt_text}, # Текстовый промпт
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}} # Картинка
            ]
        }]
