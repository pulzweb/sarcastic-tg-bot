...
# --->>> СПИСОК ТРИГГЕРОВ ДЛЯ ОГРЫЗАНИЯ <<<---
BOT_ROAST_TRIGGERS = [
    'хуй', 'пизд', 'бля', 'еблан', 'долбоеб', 'мудак',
    'тупой', 'глупый', 'дебил', 'идиот',
    'нахуй', 'в пизду', 'завали', 'заткнись',
    'бот говно', 'сломался', 'хуйня', 'бред',
    'сосал', 'жопу', 'черт', 'хуесос', 'спать', 
    'отдыхай', 'пошел ты', 'иди ты', 'отъебись', 'пошёл ты' # ))
...
# --- НОВАЯ ФУНКЦИЯ РЕАКЦИИ НА ОТВЕТЫ БОТУ (ВЕРСИЯ С GEMINI) ---
async def reply_to_bot_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Проверяет ответ на сообщение бота, и если текст содержит триггеры - генерит огрызание через Gemini."""
    # Проверки: это ответ? на сообщение? от нашего бота? есть текст?
    if not update.message or not update.message.reply_to_message or not update.message.reply_to_message.from_user: return
    if update.message.reply_to_message.from_user.id != context.bot.id: return
    if not update.message.text: return

    chat_id = update.message.chat_id
    user_text_input = update.message.text # Текст НАЕЗДА от пользователя
    user_name = update.message.from_user.first_name or "Смельчак Дерзкий"
    # Текст сообщения бота, на которое ответили (обрежем позже)
    bot_original_message_text = update.message.reply_to_message.text or "[мой старый высер без текста]"

    # Ищем триггерные слова в наезде пользователя
    triggered_word = None
    user_text_lower = user_text_input.lower()
    for trigger in BOT_ROAST_TRIGGERS: # Используем список триггеров
        if trigger in user_text_lower:
            triggered_word = trigger
            logger.info(f"Сработал триггер '{trigger}' на ответ боту от {user_name} в чате {chat_id}")
            break # Нашли один - хватит

    # Если триггер сработал - генерируем ответ
    if triggered_word:
        logger.info(f"Запрос к Gemini для генерации огрызания на фразу: '{user_text_input[:50]}...'")

        # Обрезаем тексты для промпта
        ...
        comeback_prompt = (
            ...
            f"Его фраза содержит намек на оскорбление или несогласие (слово '{triggered_word}').\n"
            f"Твоя задача: Придумай **ОДНО КОРОТКОЕ (1-2 предложения)** максимально дерзкое, язвительное, уничижительное или просто ставящее на место ОГРЫЗАНИЕ в ответ на фразу пользователя. Используй мат, сарказм, черный юмор. Покажи, что тебе абсолютно похуй на его мнение и что он выглядит жалко, пытаясь спорить с тобой. НЕ пиши вступлений. СРАЗУ выдавай свое огрызание.\n\n"
            f"Пример ответа на 'бот тупой': '🗿 А ты у нас Эйнштейн доморощенный? Пиздуй учебники читать.'\n"
            f"Пример ответа на 'завали ебало': '🗿 Ой, извини, я не знал, что твое нежное эго не выдерживает правды.'\n"
            f"Пример ответа на 'сосал?': '🗿 Твоя мамка сосала, когда тебя рожала. Вопросы?'\n\n"
        ...
        try:
            ...
            # Обработка ответа
            comeback_text = f"🗿 Пошел нахуй, {user_name}." # Заглушка
            if response.prompt_feedback.block_reason:
                block_reason = response.prompt_feedback.block_reason; logger.warning(f"Огрызание Gemini заблокировано: {block_reason}")
                comeback_text = f"🗿 Мой ответ тебе настолько охуенен, что Gemini его заблокировал ({block_reason}). Ссыкло."
            elif response.candidates:
                 try:
            ...
            # Отправляем как ответ на НАЕЗД пользователя
            ...
            # Отправим стандартную заглушку в случае ошибки генерации
            try: await update.message.reply_text(random.choice([
                "🗿 Я щас занят, иди нахуй.",
                "🗿 Твой высер недостоин моего ответа.",
                "🗿 Ошибка системы: обнаружен долбоеб. Игнорирую."
                ]))
            except Exception: pass # Если и это не отправилось - похуй

# --- КОНЕЦ ФУНКЦИИ РЕАКЦИИ НА ОТВЕТЫ (ВЕРСИЯ С GEMINI) ---
