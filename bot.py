...
async def analyze_chat(
    ...
    try:
        ...
        system_prompt = (
            ...
            f"1.  Для КАЖДОГО выделенного момента СНАЧАЛА напиши **КРАТКОЕ НЕЙТРАЛЬНОЕ ОПИСАНИЕ СУТИ**: КТО из участников (укажи имена/ники из диалога) ЧТО сделал или обсудил. Буквально одно предложение.\n"
            f"2.  ПОСЛЕ описания сути, на той же строке или следующей (как удобнее модели), добавь **КОРОТКИЙ (буквально 3-7 слов) МАКСИМАЛЬНО САРКАСТИЧНЫЙ, МАТЕРНЫЙ И УНИЧИЖИТЕЛЬНЫЙ КОММЕНТАРИЙ (панчлайн)** к этому моменту.\n"
            f"3.  **КАЖДЫЙ** такой блок (описание + панчлайн) начинай с новой строки и символа **`🗿 `** (Моаи и пробел).\n"
            f"4.  В описании сути ОБЯЗАТЕЛЬНО включай имена участников. В панчлайне можешь не повторять имена.\n"
            f"5.  Если ничего интересного не было, напиши ОДНУ строку: `🗿 Унылое болото. Ничего не произошло, кроме пускания пузырей.`\n"
            f"6.  Не пиши никаких общих вступлений или заключений.\n\n"
            f"Пример ЗАЕБАТОГО ответа:\n"
            f"🗿 Вася доказывал Пете преимущества своей новой диеты на воде и солнце. — Пиздец, скоро сдохнет, долбоеб.\n"
            f"🗿 Катя и Лена обсуждали, какого цвета трусы лучше надеть на свидание. — Охуеть какие важные проблемы решают, интеллектуалки.\n"
...
async def roast_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    roast_prompt = (
        ...
        f"Инструкции:\n"
        f"1.  Сначала подумай, над чем будешь стебаться (например: вечно ноет, умничает не по делу, странное хобби, тупит в очевидном, постоянно опаздывает, постит хуйню и т.д.).\n"
