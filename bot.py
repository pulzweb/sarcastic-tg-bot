...
async def analyze_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        prompt = (
            f"Ты - въедливый и язвительный сплетник-летописец Telegram-чата. Твоя задача - проанализировать ПОСЛЕДНИЙ фрагмент переписки, выхватить из него несколько (1-10) самых интересных моментов И ОБЯЗАТЕЛЬНО УКАЗАТЬ, КТО ИЗ УЧАСТНИКОВ (по именам/никам из диалога) что сказал или сделал в этом моменте.\n\n"
            f"Фрагмент переписки:\n"
            f"```\n{conversation_text}\n```\n\n"
            f"Инструкции для твоего ответа:\n"
            f"1.  Для КАЖДОГО выделенного момента сформулируй **КОРОТКИЙ (1-2 предложения)** комментарий в стиле **постироничного троллинга с упоминанием имен**. Используй сарказм, намеки, легкий абсурд. Поддевай конкретных участников, упоминая их имена так, как они представлены в диалоге.\n"
            f"2.  **МАТ ИСПОЛЬЗУЙ** умеренно, для усиления иронии.\n"
            f"3.  **КАЖДЫЙ** комментарий начинай с новой строки и символа **`🗿 `** (Моаи и пробел).\n"
            f"4.  **ОБЯЗАТЕЛЬНО включай имена участников**, о которых идет речь. Если имя не очевидно, не придумывай.\n"
            ...
            f"🗿 Похоже, Вася пытался убедить Петю в своих гениальных бизнес-идеях. Петя, судя по ответу, был в ахуе от такой наглости.\n"
            f"🗿 А Маша в это время очень вовремя вставила историю про своего кота. Видимо, чтобы разрядить интеллектуальную обстановку, блядь.\n\n"
            f"Выдай результат в указанном формате, НЕ ЗАБЫВАЯ ИМЕНА участников:"
        )
